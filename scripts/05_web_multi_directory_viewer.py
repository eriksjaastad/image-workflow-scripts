...
<script>
    ...
    async function processSelections() {
        ...
        try {
            ...
            if (result.success) {
                showStatus(`✅ Processed ${result.processed} operations`, 'success');
                
                // Remove processed images from DOM
                selections.forEach(sel => {
                    const item = document.querySelector(`[data-directory="${sel.directory}"][data-image="${sel.image}"]`);
                    if (item) {
                ...
                // Clear states
                Object.keys(imageStates).forEach(key => delete imageStates[key]);
        ...
        for selection in selections:
            ...
            try:
                if action == 'crop':
                    ...
                    # Also move YAML file if it exists
                    yaml_source = source_path.with_suffix('.yaml')
                    if yaml_source.exists():
                        yaml_dest = dest_path.with_suffix('.yaml')
                    ...
                    if _SEND2TRASH_AVAILABLE:
                        ...
                        # Also delete YAML file if it exists
                        yaml_source = source_path.with_suffix('.yaml')
                        if yaml_source.exists():
