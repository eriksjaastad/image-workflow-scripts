...
def analyze_real_crops(crop_dir: Path, limit: int = 100):
    ...
    # Show sample
    for _i, (img_path, w, h, _ar) in enumerate(
        zip(png_files[:20], widths[:20], heights[:20], aspect_ratios[:20], strict=False)
    ):
        pass

    # Analyze variance
    width_cv = (
        statistics.stdev(widths) / statistics.mean(widths) if len(widths) > 1 else 0
    )
    height_cv = (
        statistics.stdev(heights) / statistics.mean(heights) if len(heights) > 1 else 0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=100, help="Max images to analyze (default: 100)"
    )
