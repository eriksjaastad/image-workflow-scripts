    ...
    if selections:
        try:
            ...
            if recent:
                ...
                if days_ago > 7:
                    report.warning(
                        f"Last selection was {days_ago} days ago - no recent activity"
                    )
                else:
...
def validate_embeddings(
    ...
    if all_training_files:
        coverage = (
            (len(all_training_files) - len(missing_embeddings))
        ...
        if coverage < 95:
            report.error(
                f"Low embedding coverage ({coverage:.1f}%) - many training images missing embeddings!"
            )
        elif coverage < 99:
            report.warning(
                f"Embedding coverage is {coverage:.1f}% - some images missing embeddings"
            )
...
def main():
    ...
    _filename_cache, missing_emb_files = validate_embeddings(
        report, selection_files, crop_files
    )
