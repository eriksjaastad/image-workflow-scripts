...
def backfill_dimensions(
    dry_run: bool = True, test_mode: bool = False, test_count: int = 20
):
    """
    ...
    for idx, (row_idx, row) in enumerate(rows_to_process, 1):
        ...
        if "height_1" in rows[row_idx] and rows[row_idx].get("height_1", "") in [
            "",
            "0",
        ]:
    ...
    if not dry_run:
        if not test_mode:
            # Create backup
            backup_path = CROP_LOG.with_name(
                f"{CROP_LOG.stem}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Backfill missing dimensions in select_crop_log.csv"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Test mode (use with --count to specify rows)",
    )
    parser.add_argument(
        "--count",
        type=int,
        default=20,
        help="Number of rows to process in test mode (default: 20)",
    )
    ...
    backfill_dimensions(
        dry_run=args.dry_run, test_mode=args.test, test_count=test_count
    )
