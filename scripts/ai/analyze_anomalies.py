...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Analyze anomaly cases in training data')
    ...
    if args.export:
        ...
        with open(output_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'session_id', 'set_id', 'project', 'chosen_stage', 
                'max_rejected_stage', 'chosen_path', 'rejected_paths', 'timestamp'
            ])
            writer.writeheader()
        ...
        summary = {
            ...
            'by_project': {
                project: {
                    ...
                    'anomaly_rate': len(anomaly_list) / (len(anomaly_list) + normal_by_project[project])
                }
