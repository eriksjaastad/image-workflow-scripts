...
class FeatureExtractor:
    ...
    def _append_to_cache(self, file_path: str, embedding_hash: str, processing_time: float):
            ...
            with open(selection_log) as f:
                ...
                for row in reader:
                    ...
                    if neg_paths_str:
                        ...
                        try:
                            import json
                            neg_paths = json.loads(neg_paths_str) if neg_paths_str else []
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract features from training images (read-only)")
    parser.add_argument('--max-images', type=int, help="Limit number of images to process (for testing)")
    parser.add_argument('--force', action='store_true', help="Re-process already-cached images")
