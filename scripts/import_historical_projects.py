...
def combine_datetime(date: datetime, time_obj: Optional[time]) -> datetime:
...
def parse_csv_to_projects(csv_path: Path) -> List[Dict]:
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row_idx, row in enumerate(reader, start=1):
            ...
            initial_images = int(initial_images_str) if initial_images_str.strip() else None
            ...
            if project_name.strip():
                ...
                current_project = {
                    ...
                    'end_date': combine_datetime(date, end_time) if end_time else combine_datetime(date, start_time),
                    'initial_images': initial_images,
                    'final_images': final_images,
                ...
                if current_project and hours_num > 0:
                    current_project['end_date'] = combine_datetime(date, end_time) if end_time else combine_datetime(date, start_time)
...
def create_project_manifest(project: Dict, workspace_root: Path, dry_run: bool = True) -> Tuple[bool, str]:
    """
    ...
    created_at = project['start_date'].strftime('%Y-%m-%dT%H:%M:%S') + 'Z'  # Use start as created
...
def main():
    ...
    parser.add_argument('--commit', action='store_true',
                       help='Actually create manifest files')
    ...
    for i, project in enumerate(projects, 1):
        ...
        print(f"{i:2d}. {project_id:20s} | {start} → {end} | {days}d | {hours:4.1f}h | {initial:5d} → {final:5d} imgs")
