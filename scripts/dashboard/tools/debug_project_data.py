...
def debug_project(project_id=None):
    ...
    if project_id:
        ...
        project_ops = [op for op in all_ops if 
                      project_id.lower() in str(op.get('source_dir', '')).lower() or 
                      project_id.lower() in str(op.get('dest_dir', '')).lower() or
                      project_id.lower() in str(op.get('working_dir', '')).lower()]
        ...
        for script, ops in sorted(proj_by_script.items(), key=lambda x: len(x[1]), reverse=True):
    ...
    if project_id:
        if project_id in project_metrics:
            ...
            for tool_key, tool_data in pm.get('tools', {}).items():
                ...
                print(f"      images_per_hour: {tool_data.get('images_per_hour', 0):.2f}")
        else:
    ...
    if project_id:
        ...
        proj_overview = next((p for p in response.get('productivity_overview', []) 
                             if p.get('projectId') == project_id), None)
        ...
        # Find in table
        proj_table = next((p for p in response.get('project_productivity_table', [])
                          if p.get('projectId') == project_id), None)
        ...
        if proj_table:
            ...
            for tool_name, tool_data in proj_table.get('tools', {}).items():
                if 'images' in tool_data:
                    ...
                    print(f"    {tool_name}: {tool_data['images_total']} images in {tool_data.get('hours', 0)}h ({tool_data.get('days', 0)}d)")
                    print(f"      Selected: {tool_data.get('images_selected', 0)}, Cropped: {tool_data.get('images_cropped', 0)}")
        else:
            print(f"❌ {project_id} NOT in project_productivity_table")
    else:
        print(f"Projects in productivity_overview: {len(response.get('productivity_overview', []))}")
        print(f"Projects in project_productivity_table: {len(response.get('project_productivity_table', []))}")
