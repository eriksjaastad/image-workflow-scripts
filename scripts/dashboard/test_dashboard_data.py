    ...
    try:
        ...
        if data['file_operations_data'].get('by_script'):
            ...
            for record in data['file_operations_data']['by_script'][-5:]:  # Show last 5
                ...
                print(f"  📅 {record['time_slice']}: {script_display} → {record['file_count']:,} files")
        ...
        if data['file_operations_data'].get('by_operation'):
            ...
            for record in data['file_operations_data']['by_operation'][-5:]:  # Show last 5
                print(f"  🔧 {record['time_slice']}: {record['operation']} → {record['file_count']:,} files")
