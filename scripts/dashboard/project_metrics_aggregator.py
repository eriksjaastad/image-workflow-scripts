...
class ProjectMetricsAggregator:
    ...
    def aggregate(self) -> Dict[str, Dict[str, Any]]:
        ...
        for mf in manifests:
            ...
            ops_by_time = self._filter_ops_by_time_window(file_ops, started_at, finished_at)
            ...
            images_per_hour = round(images_processed / (max(work_minutes, 1.0) / 60.0), 2) if images_processed > 0 else 0.0
    ...
    def _filter_ops_by_time_window(self, file_ops: List[Dict[str, Any]], started_at: Optional[str], finished_at: Optional[str]) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _sum_ops(records: List[Dict[str, Any]]) -> Tuple[int, Dict[str, int], Dict[str, Dict[str, int]]]:
        total = 0
    ...
    def _apply_baselines(self, results: Dict[str, Dict[str, Any]], last_n: int = 5) -> None:
        ...
        if len(pool) >= 5:
            ...
            pool_use = pool_sorted[k:-k] if len(pool_sorted) - 2*k >= 1 else pool_sorted
        else:
            pool_use = pool
        ...
        per_tool_baseline = {tool: round(sum(vs)/len(vs), 2) for tool, vs in per_tool_vals.items() if vs}
