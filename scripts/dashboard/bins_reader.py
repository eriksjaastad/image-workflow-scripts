...
class BinsReader:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        if config_path.exists():
            try:
                ...
                pass
        return {'enabled': False, 'performance_mode': {'use_15m_bins': False, 'bin_charts': []}}
    
    def is_enabled(self) -> bool:
        """Check if bins system is enabled."""
        return self.config.get('enabled', False) and \
               self.config.get('performance_mode', {}).get('use_15m_bins', False)
                    ...
                    with open(bin_path, 'r') as f:
                        for line in f:
                            try:
                                ...
                                if project_id and bin_record.get('project_id') != project_id:
                                    continue
    ...
    def aggregate_bins_to_time_slice(
        ...
        for bin_record in bins:
            ...
            # Accumulate
            aggregated[ts_key][group_key]['file_count'] += bin_record.get('file_count', 0)
            aggregated[ts_key][group_key]['event_count'] += bin_record.get('event_count', 0)
            aggregated[ts_key][group_key]['work_seconds'] += bin_record.get('work_seconds', 0.0)
    ...
    def _format_bins_for_output(
        ...
        for bin_record in bins:
            ...
            output.append({
                ...
                'work_time_minutes': round(bin_record.get('work_seconds', 0.0) / 60.0, 2)
            })
