...
def test_label_generation():
    ...
    slices = {
        "15min": (2, 90, 200),    # 2 days lookback, expect 90-200 labels (accounting for partial days)
        "1H": (2, 24, 50),        # 2 days lookback, expect 24-50 labels (roughly 24 hours + partial)
        "D": (7, 7, 8),           # 7 days lookback, expect 7-8 labels (lookback_days includes today)
    ...
    for slice_type, (lookback, min_labels, max_labels) in slices.items():
        ...
        if min_labels <= count <= max_labels:
            print(f"  ✅ {slice_type:6s}: {count:3d} labels (expected {min_labels}-{max_labels})")
            passed += 1
        else:
            print(f"  ❌ {slice_type:6s}: {count:3d} labels (expected {min_labels}-{max_labels})")
            failed += 1
...
def test_alignment():
    ...
    if result["crop"]["counts"] == expected:
        ...
        print(f"  ❌ Counts misaligned: got {result['crop']['counts']}, expected {expected}")
        failed += 1
    ...
    if len(result["crop"]["counts"]) == len(baseline):
        ...
        print(f"  ❌ Length mismatch: {len(result['crop']['counts'])} vs {len(baseline)}")
        failed += 1
...
def test_contract_compliance():
    ...
    required_keys = [
        "metadata", "projects", "charts", "timing_data",
        "project_comparisons", "project_kpi", "project_metrics", "project_markers"
    ]
