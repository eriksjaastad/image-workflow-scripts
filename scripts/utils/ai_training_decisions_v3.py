...
def generate_group_id(project_id: str, timestamp: Optional[str] = None, batch: int = 0, index: int = 0) -> str:
    """
...
def log_ai_decision(
    ...
    image_width: int,
    image_height: int,
    ai_crop_coords: Optional[List[float]] = None,
    ...
    if not images or len(images) < 2 or len(images) > 4:
        raise ValueError(f"Invalid images list: must have 2-4 images, got {len(images)}")
    ...
    if not (0 <= ai_selected_index < len(images)):
        raise ValueError(f"Invalid ai_selected_index: {ai_selected_index} (must be 0-{len(images)-1})")
    
    if not (0 <= user_selected_index < len(images)):
        raise ValueError(f"Invalid user_selected_index: {user_selected_index} (must be 0-{len(images)-1})")
    ...
    """, (
        group_id, timestamp, project_id, directory, batch_number,
        images_json, ai_selected_index, ai_crop_json, ai_confidence,
        user_selected_index, user_action,
        None, None,  # final_crop_coords, crop_timestamp (filled later)
        image_width, image_height,
        selection_match, None  # crop_match (calculated later)
    ))
...
def update_decision_with_crop(
    ...
    if len(final_crop_coords) != 4:
        raise ValueError(f"Invalid crop coords: must be [x1, y1, x2, y2], got {final_crop_coords}")
    
    x1, y1, x2, y2 = final_crop_coords
    if not (0 <= x1 < x2 <= 1 and 0 <= y1 < y2 <= 1):
        raise ValueError(f"Invalid crop coords: {final_crop_coords} (must be in [0,1] with x1<x2, y1<y2)")
    ...
    cursor.execute("SELECT ai_crop_coords FROM ai_decisions WHERE group_id = ?", (group_id,))
    row = cursor.fetchone()
...
def calculate_crop_match(ai_crop: List[float], user_crop: List[float], tolerance: float = 0.05) -> bool:
    """
    ...
    """
    for ai_val, user_val in zip(ai_crop, user_crop):
        if abs(ai_val - user_val) > tolerance:
            return False
    return True


def calculate_crop_similarity(ai_crop: List[float], user_crop: List[float]) -> Dict[str, float]:
    """
    ...
    center_dist = ((ai_center_x - user_center_x)**2 + (ai_center_y - user_center_y)**2)**0.5
...
def validate_decision_db(db_path: Path, verbose: bool = False) -> List[str]:
    ...
    cursor.execute("SELECT COUNT(*) FROM ai_decisions WHERE timestamp IS NULL OR timestamp = ''")
    count = cursor.fetchone()[0]
    if count > 0:
        errors.append(f"Found {count} rows with missing timestamps")
    
    # Check 2: Missing user decisions
    cursor.execute("SELECT COUNT(*) FROM ai_decisions WHERE user_selected_index IS NULL")
    count = cursor.fetchone()[0]
    if count > 0:
    ...
    cursor.execute("SELECT COUNT(*) FROM ai_decisions WHERE image_width <= 0 OR image_height <= 0")
    count = cursor.fetchone()[0]
    if count > 0:
    ...
    for group_id, images_json, ai_idx, user_idx in cursor.fetchall():
        ...
        if not (0 <= ai_idx < num_images):
            errors.append(f"Invalid ai_selected_index in {group_id}: {ai_idx} (num_images={num_images})")
        if not (0 <= user_idx < num_images):
            errors.append(f"Invalid user_selected_index in {group_id}: {user_idx} (num_images={num_images})")
