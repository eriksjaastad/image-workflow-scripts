...
def generate_progress_id(tool_id: str, directory_key: str, snapshot_ts: datetime) -> str:
...
def normalize_crop_progress(raw_progress: Dict[str, Any], source_file: str) -> List[Dict[str, Any]]:
    ...
    snapshot_ts = parse_timestamp(session_start) or datetime.now(timezone.utc)
    ...
    try:
        ...
        if source_path.exists():
            ...
            fallback_ts = datetime.fromtimestamp(mtime, tz=timezone.utc)
    ...
    for dir_name, dir_data in directories.items():
        ...
        if "completed_files" in dir_data:
            ...
            snapshot["completed_files_sample"] = completed[:50] if len(completed) > 50 else completed
...
def normalize_sorter_progress(raw_progress: Dict[str, Any], source_file: str) -> Optional[Dict[str, Any]]:
    ...
    snapshot_ts = parse_timestamp(updated_at) or parse_timestamp(started_at) or datetime.now(timezone.utc)
    day_str = snapshot_ts.strftime("%Y%m%d")
    ...
    snapshot = {
        ...
        "completed_files_sample": completed_files[:50] if len(completed_files) > 50 else completed_files,
        "schema_version": "progress_v1",
        "extra": {
            "directory": directory,
            "source_file": source_file,
...
def extract_from_progress_file(progress_path: Path, tool_type: str) -> List[Dict[str, Any]]:
