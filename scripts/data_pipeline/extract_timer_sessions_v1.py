...
def normalize_session(raw_session: Dict[str, Any], source_file: str) -> Optional[Dict[str, Any]]:
    """Normalize a legacy timer session to canonical schema."""
    try:
        # Extract script_id
        script_id = raw_session.get("script_name") or raw_session.get("script", "unknown")
        ...
        if isinstance(start_time, (int, float)):
            ...
            end_dt = parse_epoch_timestamp(end_time) if end_time and end_time > 0 else None
        else:
            # String timestamp
...
def extract_from_session_file(session_path: Path) -> Optional[Dict[str, Any]]:
