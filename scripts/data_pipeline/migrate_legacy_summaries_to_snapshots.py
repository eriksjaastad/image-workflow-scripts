...
def main():
    parser = argparse.ArgumentParser(description="Migrate legacy daily summaries to snapshot format")
    ...
    print(f"Mode: {'DRY RUN (no files written)' if args.dry_run else 'LIVE (will write files)'}")
    print(f"{'=' * 70}\n")
    ...
    for legacy_file in legacy_files:
        try:
            ...
            print(f"  ✅ {day}: Migrated ({snapshot['total_events']} events, {snapshot['total_files_processed']} files, {len(snapshot['by_script'])} scripts)")
            migrated_count += 1
