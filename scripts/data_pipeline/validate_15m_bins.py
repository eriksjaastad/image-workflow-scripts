...
class BinValidator:
    ...
    def _load_bins(self) -> List[Dict[str, Any]]:
        ...
        bin_path = self.data_dir / 'aggregates' / 'daily' / f'day={self.day_str}' / 'agg_15m.jsonl'
        ...
        for idx, bin_record in enumerate(self.bins):
            for field, expected_type in required_fields.items():
                if field not in bin_record:
                    self.errors.append(ValidationError(
                        'schema',
                        f'Bin {idx}: Missing required field "{field}"'
                    ))
                    continue
    ...
    def check_work_seconds(self):
        ...
        if raw_work_seconds > 0:
            work_diff_pct = abs(bin_work_seconds - raw_work_seconds) / raw_work_seconds * 100
