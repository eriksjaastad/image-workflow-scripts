    ...
    def test_single_word_ethnicities(self):
        ...
        for prompt, expected in test_cases:
            with self.subTest(prompt=prompt):
                ...
                self.assertEqual(result, expected, f"Failed to extract '{expected}' from: {prompt}")
...
class TestAgeExtraction(unittest.TestCase):
    ...
    def test_in_her_decades_pattern(self):
        ...
        test_cases = [
            ("Woman in her mid 20s", "in_her_mid_20s"),
            ("Man in his early 30s", "in_her_early_30s"),
            ("In her late 40s", "in_her_late_40s"),
            ("In his 30s", "in_her_30s"),
        ]
...
class TestRealWorldPrompts(unittest.TestCase):
    ...
    def test_very_long_prompt(self):
        ...
        prompt = ("A breathtaking professional photograph captures a stunning petite latina woman, "
