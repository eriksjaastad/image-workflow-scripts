...
def test_javascript_syntax():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for filename in test_files:
            ...
            (test_dir / (filename.replace('.png', '.yaml'))).write_text(f"yaml: {filename}")
...
def test_keyboard_mapping_consistency():
    ...
    switch_match = re.search(r'switch\(e\.key\)\s*\{(.*?)\}', content, re.DOTALL)
    if not switch_match:
    ...
    if not re.search(r"case 'Enter'.*?nextGroup\(\)", switch_content, re.IGNORECASE | re.DOTALL):
        raise Exception("Enter key should call nextGroup()")
