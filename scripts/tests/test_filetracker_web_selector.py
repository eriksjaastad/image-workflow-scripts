...
def test_filetracker_metric_marker_and_image_only_counts(monkeypatch):
    ...
    try:
        ...
        tracker.log_metric_mode_update(
            "image_only", details="Counts reflect only .png images; companions excluded"
        )
        ...
        moved = move_file_with_all_companions(
            next(src.glob("*.png")), dest_selected, dry_run=True
        )
        # Log image-only move (1)
        tracker.log_operation(
            "move",
            source_dir=str(src.name),
            dest_dir=str(dest_selected.name),
            file_count=1,
            files=[moved[0]],
        ...
        entries = read_log()
        assert any(
            e.get("type") == "metric_mode_update" and e.get("mode") == "image_only"
            for e in entries
        )
        ...
        moved_selected = sum(
            e.get("file_count", 0)
            for e in entries
            if e.get("type") == "file_operation"
            and e.get("operation") == "move"
            and e.get("dest_dir") == str(dest_selected.name)
        )
        deletes = sum(
            e.get("file_count", 0)
            for e in entries
            if e.get("type") == "file_operation"
            and e.get("operation") in ("delete", "send_to_trash")
        )
