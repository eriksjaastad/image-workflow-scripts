...
class WebCharacterSorterTest:
    ...
    def setup(self):
        ...
        cmd = [
            ...
            "--port", "5003",  # Use different port for tests
            "--no-browser"
        ]
        ...
        try:
            ...
            if self.server_process.poll() is not None:
                stdout, stderr = self.server_process.communicate()
                raise Exception(f"Server connection failed. Server output - STDOUT: {stdout.decode()}, STDERR: {stderr.decode()}")
            else:
                raise e
    ...
    def create_test_data(self):
        ...
        for i in range(3):
            (self.test_data_dir / "person_1" / f"20250803_12000{i}_stage1_generated.png").touch()
            (self.test_data_dir / "person_1" / f"20250803_12000{i}_stage1_generated.yaml").touch()
            (self.test_data_dir / "person_2" / f"20250803_13000{i}_stage1_generated.png").touch()
            (self.test_data_dir / "person_2" / f"20250803_13000{i}_stage1_generated.yaml").touch()
    ...
    def teardown(self):
        ...
        if self.test_data_dir and self.test_data_dir.exists() and "tmp" in str(self.test_data_dir):
    ...
    def test_error_handling(self):
        ...
        assert len(severe_errors) == 0, f"Should not have severe JavaScript errors: {severe_errors}"
