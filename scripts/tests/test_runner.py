...
Usage:
    python scripts/tests/test_runner.py                    # Run all tests
    python scripts/tests/test_runner.py --safety-only      # Run only critical safety tests
    python scripts/tests/test_runner.py --performance      # Include performance tests
    python scripts/tests/test_runner.py --create-data      # Create test data first
"""
...
class TestRunner:  # not a pytest test class (has __init__)
    ...
    def test_sequential_singletons_edge_case(self):
        ...
        # Parse the summary line (format: "Total: X triplets, Y pairs, Z singletons (N groups)")
        total_groups = 0
        for line in lines:
            if line.startswith("Total:"):
                ...
                match = re.search(r"\((\d+) groups\)", line)
                if match:
        ...
        if total_groups != 4:
            ...
            print(
                "This indicates the sequential grouping algorithm is not working correctly!"
            )
    ...
    def test_subdirectory_scanning(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if "15 groups" not in total_text and "groups" not in total_text:
                print(
                    f"Expected to find many groups from subdirectories, got: {total_text}"
                )
                return False
            ...
            if group_count < 10:  # Should find many groups across subdirectories
                print(
                    f"Expected many groups from subdirectories, only found {group_count}"
                )
                return False

            print(
                f"✓ Recursive scanning found {group_count} groups across subdirectories"
            )
            return True
    ...
    def test_non_standard_files_robustness(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.returncode != 0:
                print(
                    f"Script failed when processing non-standard files: {result.stderr}"
                )
                return False
            ...
            if group_count < 10:  # Should still find many valid groups
                print(
                    f"Expected many valid groups despite random files, only found {group_count}"
                )
                return False

            print(
                f"✓ Gracefully processed {group_count} valid groups while ignoring non-standard files"
            )
            return True
    ...
    def test_file_safety(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Try to run analysis (should not modify files)
            # Note: The script requires Flask, so we handle ImportError gracefully
            try:
                result = subprocess.run(
                    [
                        sys.executable,
                ...
                # If script runs successfully, check that no files were modified
                if result.returncode == 0:
                    for file_path in test_dir.glob("*"):
    ...
    def test_desktop_image_selector_crop(self):
        ...
        try:
            ...
            test_script_path = (
                Path(__file__).parent / "test_desktop_image_selector_crop.py"
            )
            result = subprocess.run(
    ...
    def test_dashboard(self):
        ...
        try:
            ...
            test_script_path = Path(__file__).parent / "test_dashboard.py"
            result = subprocess.run(
                [sys.executable, str(test_script_path)],
                capture_output=True,
                text=True,
    ...
    def summary(self):
        ...
        critical_failed = len(
            [
                ...
                if r.get("critical") and r["status"] != "PASSED"
            ]
        )
...
def main():
    ...
    parser.add_argument(
        "--safety-only", action="store_true", help="Run only critical safety tests"
    )
    parser.add_argument(
        "--performance", action="store_true", help="Include performance tests"
    )
    parser.add_argument(
        "--create-data", action="store_true", help="Create test data first"
    )
    ...
    if not runner.run_test(
        "Subdirectory Scanning Test", runner.test_subdirectory_scanning, critical=True
    ):
        return 1

    if not runner.run_test(
        "Non-Standard Files Robustness",
        runner.test_non_standard_files_robustness,
        critical=True,
    ):
        return 1

    if not runner.run_test(
        "Batch Processing Safety", runner.test_batch_processing_safety, critical=True
    ):
        return 1
    ...
    if not args.safety_only:
        ...
        runner.run_test(
            "Sequential Singletons Edge Case",
            runner.test_sequential_singletons_edge_case,
        )
        runner.run_test("UI Integrity Test", runner.test_ui_integrity)
        runner.run_test(
            "Web Tools Comprehensive Test", runner.test_web_tools_comprehensive
        )
        runner.run_test(
            "Desktop Image Selector Crop Test", runner.test_desktop_image_selector_crop
        )
