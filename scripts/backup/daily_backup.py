...
def main():
    ...
    parser.add_argument("--dest", default=str(Path.home() / "project-data-archives" / "image-workflow"))
    ...
    manifest["items"].append({
        ...
        **copy_tree(data_root / 'file_operations_logs', out_dir / 'file_operations_logs')
    })
    ...
    manifest["items"].append({
        ...
        **copy_tree(data_root / 'snapshot' / 'operation_events_v1', out_dir / 'snapshot' / 'operation_events_v1')
    })
    manifest["items"].append({
        "category": "snapshot_daily_aggregates_v1",
        **copy_tree(data_root / 'snapshot' / 'daily_aggregates_v1', out_dir / 'snapshot' / 'daily_aggregates_v1')
    })
    manifest["items"].append({
        "category": "snapshot_derived_sessions_v1",
        **copy_tree(data_root / 'snapshot' / 'derived_sessions_v1', out_dir / 'snapshot' / 'derived_sessions_v1')
    })

    # 4) ai_data backups and DBs
    manifest["items"].append({
        "category": "ai_data_backups",
        **copy_tree(data_root / 'ai_data' / 'backups', out_dir / 'ai_data' / 'backups')
    })
