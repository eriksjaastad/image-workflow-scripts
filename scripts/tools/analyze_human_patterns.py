...
def analyze_human_patterns(csv_path):
        ...
        for row in reader:
            if row['timestamp']:
                crops.append({
                    ...
                    'image_count': int(row['image_count']) if row['image_count'] else 1
                })
    ...
    for session in sessions:
        ...
        for quarter in quarters:
            if len(quarter) < 2:
                continue
            duration = (quarter[-1]['timestamp'] - quarter[0]['timestamp']).total_seconds() / 60
            if duration > 0:
    ...
    if session_speed_changes:
        avg_quarters = [
            statistics.mean([s[i] for s in session_speed_changes])
            for i in range(4)
        ]
        print("\nAverage crops/hour by session quarter:")
        for i, speed in enumerate(avg_quarters, 1):
            pct_change = ((speed - avg_quarters[0]) / avg_quarters[0] * 100) if avg_quarters[0] > 0 else 0
    ...
    for hour in sorted(hourly_crops.keys()):
        ...
        while i < len(crop_list):
            ...
            while j < len(crop_list):
                if (crop_list[j]['timestamp'] - crop_list[j-1]['timestamp']).total_seconds() < 600:
            ...
            if len(seq) >= 2:
                duration = (seq[-1]['timestamp'] - seq[0]['timestamp']).total_seconds() / 60
                if duration > 0:
    ...
    timing_data = {
        ...
        } if within_session_times else {},
        "mean": float(statistics.mean(within_session_times)) if within_session_times else 0,
        "stddev": float(statistics.stdev(within_session_times)) if within_session_times else 0,
