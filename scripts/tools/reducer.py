...
def parse_args(argv=None):
    ...
    common.add_argument("--group-window", choices=["2m","5m","10m"], default=DEFAULTS["group_window"]) 
    common.add_argument("--dedupe", type=float, choices=[0.90,0.85,0.80], default=DEFAULTS["dedupe"]) 
...
def cmd_run(args):
    ...
    try:
        ...
        if args.simulate_hang:
            ...
            group_iter = groups
            if args.limit:
                group_iter = groups[: int(args.limit)]

            dst_selected = root.parent / "selected"
            dst_delete = root.parent / "delete"
            dst_crop = root.parent / "crop"
            if args.commit:
