...
def main() -> None:
    ...
    parser.add_argument("--limit", type=int, default=10, help="Max number of images to enqueue")
    parser.add_argument("--dest", dest="dest_dir", default="__cropped", help="Destination directory for crops")
    parser.add_argument("--session", dest="session_id", default="enqueue_test", help="Session ID")
    parser.add_argument("--project", dest="project_id", default="debug", help="Project ID")
    ...
    cq = CropQueueManager()
    batch_id = cq.enqueue_batch(crops, session_id=args.session_id, project_id=args.project_id)
